...
const mongoConnections: Map<string, MongoConnection> = new Map();
...
async function testMongoConnection(config: MongoConnectionConfig): Promise<{ success: boolean; error?: string }> {
    if (!mongodb) return { success: false, error: 'MongoDB 驱动未安装，请运行: npm install mongodb' };
...
async function connectMongo(config: MongoConnectionConfig): Promise<{ success: boolean; connectionId?: string; error?: string }> {
    if (!mongodb) return { success: false, error: 'MongoDB 驱动未安装' };
...
async function disconnectMongo(connectionId: string): Promise<{ success: boolean; error?: string }> {
    try {
...
async function mongoListDatabases(connectionId: string): Promise<{ success: boolean; databases?: string[]; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
async function mongoListCollections(connectionId: string, database: string): Promise<{ success: boolean; collections?: string[]; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
async function mongoGetCollectionStats(connectionId: string, database: string, collection: string): Promise<{ success: boolean; stats?: { count: number; size: number; avgObjSize: number }; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
async function mongoFindDocuments(connectionId: string, database: string, collection: string, filterStr: string, sortStr: string, skip: number, limit: number): Promise<{ success: boolean; documents?: any[]; total?: number; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
async function mongoInsertDocument(connectionId: string, database: string, collection: string, documentStr: string): Promise<{ success: boolean; insertedId?: string; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
async function mongoUpdateDocument(connectionId: string, database: string, collection: string, id: string, documentStr: string): Promise<{ success: boolean; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
async function mongoDeleteDocument(connectionId: string, database: string, collection: string, id: string): Promise<{ success: boolean; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
async function mongoGetIndexes(connectionId: string, database: string, collection: string): Promise<{ success: boolean; indexes?: Array<{ name: string; key: Record<string, number> }>; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
async function mongoRunCommand(connectionId: string, database: string, commandStr: string): Promise<{ success: boolean; result?: any; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
async function mongoDropCollection(connectionId: string, database: string, collection: string): Promise<{ success: boolean; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
async function mongoCreateCollection(connectionId: string, database: string, collection: string): Promise<{ success: boolean; error?: string }> {
    try {
        const conn = mongoConnections.get(connectionId);
        if (!conn) return { success: false, error: '连接不存在' };
...
function setupMongoHandlers() {
    ipcMain.handle('mongo:test-connection', async (_e, config: MongoConnectionConfig) => testMongoConnection(config));
    ipcMain.handle('mongo:connect', async (_e, config: MongoConnectionConfig) => connectMongo(config));
    ipcMain.handle('mongo:disconnect', async (_e, connectionId: string) => disconnectMongo(connectionId));
    ipcMain.handle('mongo:list-databases', async (_e, connectionId: string) => mongoListDatabases(connectionId));
    ipcMain.handle('mongo:list-collections', async (_e, connectionId: string, database: string) => mongoListCollections(connectionId, database));
    ipcMain.handle('mongo:get-collection-stats', async (_e, connectionId: string, database: string, collection: string) => mongoGetCollectionStats(connectionId, database, collection));
    ipcMain.handle('mongo:find-documents', async (_e, connectionId: string, database: string, collection: string, filter: string, sort: string, skip: number, limit: number) => mongoFindDocuments(connectionId, database, collection, filter, sort, skip, limit));
    ipcMain.handle('mongo:insert-document', async (_e, connectionId: string, database: string, collection: string, document: string) => mongoInsertDocument(connectionId, database, collection, document));
    ipcMain.handle('mongo:update-document', async (_e, connectionId: string, database: string, collection: string, id: string, document: string) => mongoUpdateDocument(connectionId, database, collection, id, document));
    ipcMain.handle('mongo:delete-document', async (_e, connectionId: string, database: string, collection: string, id: string) => mongoDeleteDocument(connectionId, database, collection, id));
    ipcMain.handle('mongo:get-indexes', async (_e, connectionId: string, database: string, collection: string) => mongoGetIndexes(connectionId, database, collection));
    ipcMain.handle('mongo:run-command', async (_e, connectionId: string, database: string, command: string) => mongoRunCommand(connectionId, database, command));
    ipcMain.handle('mongo:drop-collection', async (_e, connectionId: string, database: string, collection: string) => mongoDropCollection(connectionId, database, collection));
    ipcMain.handle('mongo:create-collection', async (_e, connectionId: string, database: string, collection: string) => mongoCreateCollection(connectionId, database, collection));
}
